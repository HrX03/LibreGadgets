package dev.hrx.libregadgets.core.api

import android.content.Context
import android.util.Log
import dev.hrx.libregadgets.core.api.types.ConnectionResponse
import dev.hrx.libregadgets.core.api.types.CountryResponse
import dev.hrx.libregadgets.core.api.types.LoginBody
...
private class TokenInterceptor(context: Context) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if(storage.accountId.isNotEmpty()) {
            val digest = MessageDigest.getInstance("SHA-256")
            val accId = storage.accountId.encodeToByteArray()
            val encoded = digest.digest(accId)
            request = request
                .newBuilder()
                .header("Account-Id", encoded.toHexString(HexFormat.UpperCase))
                .build()
        }
